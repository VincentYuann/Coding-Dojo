...
function AnimeCard({ anime }) {
    ...
    function handleFavoriteClick() {
        if (isFavorite) {
            ...
            console.log("Removed from favorites:", anime.title_english || anime.title || anime.titles[0].title, anime.mal_id);
        } else {
            // Add the new 'anime' to favorites
            setFavorites(previousFavorites => [...previousFavorites, anime]);
            console.log("Added to favorites:", anime.title_english || anime.title || anime.titles[0].title, anime.mal_id);
        }
    }
